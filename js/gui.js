	...
	this.show_action_attributes = function () {
		...
		for (var k in this.action_data().attributes) {
			...
			if (this.action_data().attributes[k] === true || this.action_data().attributes[k] === false) {
				...
				html += '<select class="brushop" onchange="GUI.update_attribute(this);" id="' + k + '">';
				...
				html += '</tr>';
				html += '</table>';
				html += '<div style="float:left;width:80px;" onclick="GUI.update_attribute(this, ' + (step) + ')" class="attribute-area" id="' + k + '">+</div>';
